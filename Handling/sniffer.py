...
def check_packet(payload, src_ip, dst_port):
    """Checks packet against defined rules with performance tracking.
    ...
    try:
        for rule in RULES:
            # Optimized port check first (faster than string search)
        ...
        detection_time = time.time() - detection_start
        log_event(f"Error during packet analysis: {str(e)}", event_type="ERROR")
        return None, detection_time
